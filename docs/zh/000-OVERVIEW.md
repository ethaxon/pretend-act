...
Pretend Act 是一个本地 workflow simulation 工具包，面向那些需要的不只是 shell 调用 `act` 的项目。它提供程序化 runner、一次性 GitHub Actions 依赖 container、workflow overlay、artifact helper，以及 release automation 可复用的 registry 配置 helper。

首轮实现是 practical implementation，聚焦真实 release workflow simulation 需求，尤其覆盖 SecurityDept release workflow simulation 的使用方式，同时保持公开模型不绑定单一 runner engine。
...
| GitHub Actions container | 支持 | 临时 repository source projection、`injection-js` provider 和依赖 cleanup。 |
...
| remote-mock publish mode | 部分支持 | 已支持简单 run step 的 npm/Cargo publish pretender；Docker publish rewrite 需要显式 opt in。 |
